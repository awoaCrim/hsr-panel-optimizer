    ...
    for f in sorted(NORMALIZED_DIR.glob("*.json")):
        ...
        for path, value, prov in _leaf_paths(doc, f.name.replace(".json", "")):
            ...
            if prov.version:
                ...
                elif not prov.version.startswith(("biligame-", "miyoushe-", "nanoka-")):
                    errors.append(f"{path}: version={prov.version!r} 格式未知（应 srr@/tbgd@<sha 前 7 位> 或 biligame-/miyoushe-/nanoka-<日期/版本>）")
            # 未验证值收集
