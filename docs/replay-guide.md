...
`data/enemy_starforge12b.json` = **最新忘却之庭·星启模式 第12关 第2节点**
（StageConfig 30124122 合金机铠·帕姆王；数据源 TBGD StageConfig/MonsterConfig/MonsterTemplateConfig）：

- **波次1**：破晓战队·苍翼（量子抗80%！）+ 破晓战队·灰烬（物理抗80%！）——成对出场
- **波次2**：合金机铠·帕姆王（Boss，131万HP，全抗20%）
- Lv95 数值 = 模板基础值 × HardLevelGroup 3 比例（HP×375.4385/ATK×34.75065/SPD×1.32）
- 技能倍率 = MonsterSkillConfig ParamList[0]（分布推断，待实测）；技能削韧字段未解包（UNKNOWN）
- target_av=1000（星启3节点共享轮数未知，棱彩星=剩余33轮以上；单节点≈10轮近似，待实测）

**多波次（D8 已完成）**：enemy 文件支持 `waves` 字段；波次推进 = 全灭瞬间切换
（事件入流、HP/韧性/冷却重建、行动条满速入队）；快照/回退跨波次一致。
同时修复死敌队列 bug：敌人死亡即移出行动队列（原为残留行动到超时）。
