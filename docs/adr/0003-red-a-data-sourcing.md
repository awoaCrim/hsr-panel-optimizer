...
- **天赋补充**：红A 追击回能 5（此前遗漏）
- **额外能力行迹补充**：`AvatarSkillTreeConfigLD` 的 `PointID=1015101`（「投影魔术」）`ParamList[0]=2`，Nanoka 4.4.54 同 ID 中文文本为“Archer在场时，战技点上限额外增加2点”。该效果属于额外能力行迹，不是「心眼（真）」天赋正文；已结构化为 `sp_cap_bonus: 2`
- 待回填：TurnBasedGameData 更新联动技能数据后以解包值为准
